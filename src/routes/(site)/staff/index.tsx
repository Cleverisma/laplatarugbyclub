...
export const useStaffLoader = routeLoader$(async (requestEvent) => {
  ...
  const categories = ['Plantel Superior', 'Juvenil', 'Infantil', 'Escuelita'] as const;
  ...
  divisionsData.forEach(division => {
    const group = division.groupType as string;
...
export default component$(() => {
  ...
  const groupedData = dataLoader.value;

  const categories = ['Plantel Superior', 'Juvenil', 'Infantil', 'Escuelita'];
        ...
        <div class="container mx-auto px-4 max-w-[95vw] lg:max-w-6xl relative z-30">
          ...
          <div class="animate-[fadeIn_0.5s_ease-out]">
            {(() => {
              ...
              const { subcomision, coaching, equipos } = plantData;

              // Check if entire plantel is empty
              const totalDivisionsCount = subcomision.length + coaching.length + equipos.length;
              ...
              return (
                <div class="space-y-24">
                  ...
                  {/* 2. DEPARTAMENTO DE COACHING (Nested) */}
                  {coaching.length > 0 && (
                    <div>
                      ...
                      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                         {coaching.map(division => (
                            division.staffMembers.map(member => (
                              <div key={member.id} class="bg-white/5 border border-white/10 p-5 rounded-lg text-center hover:bg-white/10 transition-colors">
                  ...
                  {equipos.length > 0 && (
                    <div>
                      <div class="flex items-center justify-center mb-10">
                         <h2 class="text-xl font-bold text-white uppercase tracking-[0.2em]" style={{ fontFamily: "'Oswald', sans-serif" }}>EQUIPOS / DIVISIONES</h2>
                      </div>
                      
                      <div class="space-y-10">
                        {equipos.map((division) => (
                          <div key={division.id} class="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-6 md:p-8">
                            <h3
