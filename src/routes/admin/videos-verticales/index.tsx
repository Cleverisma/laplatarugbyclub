...
export const useVerticalVideosAdminLoader = routeLoader$(async (requestEvent) => {
  const db = getDb(requestEvent.env);
  return db.select().from(verticalVideos).orderBy(asc(verticalVideos.displayOrder));
});
...
export const head: DocumentHead = {
  title: 'Gestión de Videos Verticales — Admin | La Plata Rugby Club',
};
    ...
    <div>
      ...
      <div class="flex items-center justify-between mb-8">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">
            {videosList.length} video{videosList.length !== 1 ? 's' : ''} registrado{videosList.length !== 1 ? 's' : ''}
          </p>
        </div>
        <Link
      ...
      {videosList.length === 0 && (
        <div class="bg-white rounded-xl border border-gray-200 p-16 text-center">
          ...
          <p class="text-gray-500 font-medium">No hay videos registrados aún.</p>
          <Link href="/admin/videos-verticales/new" class="inline-block mt-4 text-sm text-yellow-600 font-semibold hover:underline">
            + Crear primer video
          </Link>
        </div>
      )}
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="text-xs text-gray-400 uppercase tracking-widest border-b border-gray-100">
                  <th class="text-left px-6 py-3 font-semibold w-16">ID</th>
                  <th class="text-left px-6 py-3 font-semibold w-24">Miniatura</th>
              ...
              <tbody>
                {videosList.map((video, idx) => (
                  ...
                  >
                    <td class="px-6 py-3 text-gray-400 font-mono text-xs">{video.id.slice(0, 8)}...</td>
                    <td class="px-6 py-3">
                      <div class="w-10 h-16 rounded overflow-hidden bg-gray-100 flex items-center justify-center">
                        {video.thumbnailUrl ? (
                           <img src={video.thumbnailUrl} alt={video.title} class="w-full h-full object-cover" loading="lazy" />
                        ) : (
                           <span class="text-[10px] text-gray-400">Sin img</span>
                        )}
                      </div>
                    </td>
                    <td class="px-6 py-3">
                      <span class="font-semibold text-gray-900">{video.title}</span>
                    </td>
                    <td class="px-6 py-3">
                      {video.isActive === 1 ? (
                    ...
                    <td class="px-6 py-3 text-gray-500 font-mono text-xs">{video.displayOrder}</td>
                    <td class="px-6 py-3">
