...
export const useVerticalVideoLoader = routeLoader$(async (requestEvent) => {
  ...
  const video = await db.query.verticalVideos.findFirst({
...
export const useEditVerticalVideoAction = routeAction$(
  async (data, requestEvent) => {
    ...
    await db.update(verticalVideos).set({
...
export const head: DocumentHead = {
  title: 'Editar Video Vertical — Admin | La Plata Rugby Club',
};
    ...
    <div>
      ...
      <div class="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          >
            Editar Video Vertical
          </h1>
          <p class="text-sm text-gray-500 mt-1">
            Modificar información del video seleccionado.
          </p>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-8 max-w-2xl">
        {editAction.value?.failed && (
          <div class="bg-red-50 text-red-700 text-sm px-4 py-3 rounded-lg mb-6 border border-red-200">
            {(editAction.value as any).error || 'Por favor corregí los errores en el formulario.'}
        ...
        >
          ...
          <div class="p-5 border border-gray-200 rounded-lg bg-gray-50/50 space-y-4">
            ...
            <div>
              <label for="videoFile" class="block text-sm text-gray-700 mb-2">Subir Nuevo Video (MP4)</label>
              <input
                type="file"
                id="videoFile"
                name="videoFile"
              ...
              />
              <p class="text-xs text-gray-400 mt-1">Si subis un nuevo archivo, reemplazará a la URL actual.</p>
            </div>
            <div>
              <label for="videoUrl" class="block text-sm text-gray-700 mb-2">URL del Video (Actual)</label>
              <input
                type="text"
                id="videoUrl"
                name="videoUrl"
          ...
          <div class="p-5 border border-gray-200 rounded-lg bg-gray-50/50 space-y-4">
            ...
            <div>
              <label for="thumbnailFile" class="block text-sm text-gray-700 mb-2">Subir Nueva Miniatura</label>
              <input
                type="file"
                id="thumbnailFile"
                name="thumbnailFile"
            ...
            <div>
              <label for="thumbnailUrl" class="block text-sm text-gray-700 mb-2">URL de Miniatura (Actual)</label>
              <input
                type="text"
                id="thumbnailUrl"
                name="thumbnailUrl"
